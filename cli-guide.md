...
```bash
bun run index.ts <root-command> <action> [options]
```
...
bun run index.ts market ticker --help
```
...
```bash
bun run index.ts market depth --exchange binance --symbol BTCUSDT --levels 10
```
...
```bash
bun run index.ts market kline --exchange binance --symbol BTCUSDT --interval 1m --limit 50
```
...
```bash
bun run index.ts account snapshot --exchange standx
bun run index.ts account summary --exchange standx
```
...
```bash
bun run index.ts position list --exchange standx
bun run index.ts position list --exchange standx --symbol BTC-USD
```
...
```bash
bun run index.ts order open --exchange binance --symbol BTCUSDT
```

### Create order

```bash
bun run index.ts order create --exchange binance --symbol BTCUSDT --side buy --type limit --quantity 0.01 --price 90000
```
...
```bash
# Market order
bun run index.ts order create --exchange binance --symbol BTCUSDT --side buy --type market --qty 0.01

# Stop order
bun run index.ts order create --exchange binance --symbol BTCUSDT --side sell --type stop --qty 0.01 --stop-price 86000

# Trailing-stop order
bun run index.ts order create --exchange binance --symbol BTCUSDT --side sell --type trailing-stop --qty 0.01 --activation-price 91000 --callback-rate 0.2

# Close intent order (defaults reduceOnly/closePosition in route layer)
bun run index.ts order create --exchange binance --symbol BTCUSDT --side sell --type close --qty 0.01
```

### Cancel order

```bash
bun run index.ts order cancel --exchange binance --symbol BTCUSDT --order-id 123456
```

### Cancel all orders

```bash
bun run index.ts order cancel-all --exchange binance --symbol BTCUSDT
```
...
```bash
bun run index.ts order create --exchange binance --symbol BTCUSDT --side buy --type limit --quantity 0.01 --price 90000 --dry-run --json
```
...
bun run index.ts exchange capabilities --exchange binance --json

# 2) Fetch market state
bun run index.ts market ticker --exchange binance --symbol BTCUSDT --json

# 3) Validate order parameters with dry-run
bun run index.ts order create --exchange binance --symbol BTCUSDT --side buy --type limit --quantity 0.01 --price 90000 --dry-run --json

# 4) Submit live order after validation (remove --dry-run)
bun run index.ts order create --exchange binance --symbol BTCUSDT --side buy --type limit --quantity 0.01 --price 90000 --json
```
