...
describe('AsterSpotRestClient', () => {
   ...
   it('signs market order requests', async () => {
      ...
      expect(init.headers).toEqual({ 'X-MBX-APIKEY': 'key', 'Content-Type': 'application/x-www-form-urlencoded' });
   ...
   it('attaches api key for historical trades without signing', async () => {
      const trades = [{ id: 1, price: '1', qty: '1', time: 1000, isBuyerMaker: false }];
