...
import type { OrderLockMap, OrderPendingMap, OrderTimerMap } from '../src/core/order-coordinator';
import { deduplicateOrders, marketClose, placeMarketOrder, placeOrder, placeStopLossOrder, placeTrailingStopOrder, unlockOperating } from '../src/core/order-coordinator';
...
describe('order-coordinator', () => {
   it('deduplicates orders by type and side', async () => {
      ...
      const log = vi.fn();
      const openOrders: AsterOrder[] = [{ ...baseOrder, orderId: 1 }, { ...baseOrder, orderId: 2 }];
   ...
   it('places limit orders and records pending id', async () => {
      ...
      const pending: OrderPendingMap = {};
      const log = vi.fn();
      await placeOrder(adapter, 'BTCUSDT', [], locks, timers, pending, 'BUY', '100', 1, log, false);
   ...
   it('places market order and unlocks after completion', async () => {
      ...
      const pending: OrderPendingMap = {};
      const log = vi.fn();
      await placeMarketOrder(adapter, 'BTCUSDT', [], locks, timers, pending, 'SELL', 1, log, true);
      expect(adapter.createOrder).toHaveBeenCalled();
   ...
   it('places stop loss order only when valid', async () => {
      ...
      const pending: OrderPendingMap = {};
      const log = vi.fn();
      await placeStopLossOrder(adapter, 'BTCUSDT', [], locks, timers, pending, 'SELL', 99, 1, 100, log);
      expect(adapter.createOrder).toHaveBeenCalled();
   ...
   it('places trailing stop order', async () => {
      ...
      const pending: OrderPendingMap = {};
      const log = vi.fn();
      await placeTrailingStopOrder(adapter, 'BTCUSDT', [], locks, timers, pending, 'SELL', 101, 1, 0.2, log);
      expect(adapter.createOrder).toHaveBeenCalled();
   ...
   it('market close cancels open orders before placing close order', async () => {
      ...
      const pending: OrderPendingMap = {};
      const log = vi.fn();
      await marketClose(adapter, 'BTCUSDT', [{ ...baseOrder, orderId: 2 }], locks, timers, pending, 'SELL', 1, log);
      expect(adapter.createOrder).toHaveBeenCalled();
