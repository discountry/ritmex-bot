...
const mockSnapshot = (positions: Array<{ symbol: string; amt: number; entry: number; pnl: number }> = []): AsterAccountSnapshot => ({
   ...
   positions: positions.map((p) => ({ symbol: p.symbol, positionAmt: String(p.amt), entryPrice: String(p.entry), unrealizedProfit: String(p.pnl), positionSide: 'BOTH', updateTime: Date.now() })),
   assets: [],
});

const mockKlines = (values: number[]): AsterKline[] => values.map((value, index) => ({ openTime: index, open: String(value), high: String(value), low: String(value), close: String(value), volume: '0', closeTime: index + 1, numberOfTrades: 0 }));
