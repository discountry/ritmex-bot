...
export class OffsetMakerEngine {
   ...
   private lastEntryOrderBySide: Record<'BUY' | 'SELL', { price: string; ts: number } | null> = { BUY: null, SELL: null };
   ...
   private async enforceRateLimitStop(): Promise<void> {
      ...
      try {
         await marketClose(this.exchange, this.config.symbol, this.openOrders, this.locks, this.timers, this.pending, side, absPosition, (type, detail) => this.tradeLog.push(type, detail), {
   ...
   private evaluateDepth(depth: AsterDepth): { buySum: number; sellSum: number; skipBuySide: boolean; skipSellSide: boolean; imbalance: 'balanced' | 'buy_dominant' | 'sell_dominant' } {
      // Keep existing behavior: 10 levels, ratio threshold 3x
      return computeDepthStats(depth, 10, 3);
   }

   private async handleImbalanceExit(position: PositionSnapshot, buySum: number, sellSum: number): Promise<boolean> {
      const absPosition = Math.abs(position.positionAmt);
      ...
      this.tradeLog.push('stop', `深度极端不平衡(${buySum.toFixed(4)} vs ${sellSum.toFixed(4)}), 市价平仓 ${side}`);
      try {
         await this.flushOrders();
         await marketClose(this.exchange, this.config.symbol, this.openOrders, this.locks, this.timers, this.pending, side, absPosition, (type, detail) => this.tradeLog.push(type, detail), {
            markPrice: position.markPrice,
   ...
   private async syncOrders(targets: DesiredOrder[]): Promise<void> {
      ...
      for (let i = 0; i < adjustedTargets.length; i++) {
         ...
         if (!t || t.reduceOnly) {
            continue; // only suppress entry orders
         }
         ...
         if (ticksDiff < this.minRepriceTicks || withinDwell) {
            ...
            adjustedTargets[i] = { side: t.side, price: String(existing.price), amount: t.amount, reduceOnly: false };
         }
      }
