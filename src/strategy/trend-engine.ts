...
import { marketClose, placeMarketOrder, placeStopLossOrder, placeTrailingStopOrder, unlockOperating } from '../core/order-coordinator';
...
import { calcStopLossPrice, calcTrailingActivationPrice, computeBollingerBandwidth, getPosition, getSMA, type PositionSnapshot } from '../utils/strategy';
...
export class TrendEngine {
   ...
   private lastAccountPosition: PositionSnapshot = { positionAmt: 0, entryPrice: 0, unrealizedProfit: 0, markPrice: null };
   ...
   private bootstrap(): void {
      ...
      safeSubscribe<AsterAccountSnapshot>(
         ...
         log,
         { subscribeFail: (error) => `订阅账户失败: ${String(error)}`, processFail: (error) => `账户推送处理异常: ${extractMessage(error)}` },
      );
      ...
      safeSubscribe<AsterOrder[]>(
         ...
         (orders) => {
            ...
            this.openOrders = Array.isArray(orders) ? orders.filter((order) => order.type !== 'MARKET' && order.symbol === this.config.symbol && isActive(order.status)) : [];
         ...
         log,
         { subscribeFail: (error) => `订阅订单失败: ${String(error)}`, processFail: (error) => `订单推送处理异常: ${extractMessage(error)}` },
      );
      ...
      safeSubscribe<AsterDepth>(
         ...
         log,
         { subscribeFail: (error) => `订阅深度失败: ${String(error)}`, processFail: (error) => `深度推送处理异常: ${extractMessage(error)}` },
      );
      ...
      safeSubscribe<AsterTicker>(
         ...
         log,
         { subscribeFail: (error) => `订阅Ticker失败: ${String(error)}`, processFail: (error) => `价格推送处理异常: ${extractMessage(error)}` },
      );
      ...
      safeSubscribe<AsterKline[]>(
         ...
         log,
         { subscribeFail: (error) => `订阅K线失败: ${String(error)}`, processFail: (error) => `K线推送处理异常: ${extractMessage(error)}` },
      );
   }
   ...
   private isReady(): boolean {
      ...
      return Boolean(this.accountSnapshot && this.tickerSnapshot && this.depthSnapshot && this.klineSnapshot.length >= minKlines);
   }
   ...
   private logKlineSnapshot(): void {
      ...
      if (count < minKlines) {
         if (!this.klineInsufficientLogged) {
            ...
            this.tradeLog.push('info', `K线不足 ${count}/${minKlines}，最近收盘(${closes.length}): ${closes.join(', ')}`);
      ...
      if (!this.klineReadyLogged) {
         const closes = this.klineSnapshot.slice(-5).map((k) => Number(k.close).toFixed(2));
         this.tradeLog.push('info', `K线就绪 ${count} 根，可计算 SMA30。最近收盘: ${closes.join(', ')}`);
   ...
   private async tick(): Promise<void> {
      ...
      try {
         ...
         const bollingerBandwidth = computeBollingerBandwidth(this.klineSnapshot, this.config.bollingerLength, this.config.bollingerStdMultiplier);
   ...
   private async enforceRateLimitStop(): Promise<void> {
      ...
      let referencePrice = this.getReferencePrice();
      if ((!referencePrice || referencePrice === null) && this.tickerSnapshot) {
         referencePrice = Number(this.tickerSnapshot.lastPrice);
      }
   ...
   private logStartupState(): void {
      ...
      if (hasPosition) {
         this.tradeLog.push('info', `检测到已有持仓: ${position.positionAmt > 0 ? '多' : '空'} ${Math.abs(position.positionAmt).toFixed(4)} @ ${position.entryPrice.toFixed(2)}`);
      }
   ...
   private async handleOpenPosition(currentPrice: number, currentSma: number, currentBandwidth: number | null): Promise<void> {
      ...
      if (Number.isFinite(currentBandwidth) && this.config.minBollingerBandwidth > 0 && Number(currentBandwidth) < this.config.minBollingerBandwidth) {
         if (now - this.lastBollingerBlockLogged > 15_000) {
            this.tradeLog.push('info', `布林带宽度不足：${Number(currentBandwidth).toFixed(4)} < ${this.config.minBollingerBandwidth}，忽略入场信号`);
   ...
   private async submitMarketOrder(side: 'BUY' | 'SELL', price: number, reason: string): Promise<void> {
      try {
         await placeMarketOrder(this.exchange, this.config.symbol, this.openOrders, this.locks, this.timers, this.pending, side, this.config.tradeAmount, (type, detail) => this.tradeLog.push(type, detail), false, {
   ...
   private async handlePositionManagement(position: PositionSnapshot, price: number): Promise<{ closed: boolean; pnl: number }> {
      ...
      const effectiveClosePrice = Number.isFinite(closeSidePriceRaw) ? closeSidePriceRaw : Number.isFinite(price) ? price : position.entryPrice;
      const pnl = qtyAbs > 0 ? (direction === 'long' ? effectiveClosePrice - position.entryPrice : position.entryPrice - effectiveClosePrice) * qtyAbs : 0;
      const unrealized = Number.isFinite(position.unrealizedProfit) ? position.unrealizedProfit : null;
      const stopSide = direction === 'long' ? 'SELL' : 'BUY';
      const stopPrice = calcStopLossPrice(position.entryPrice, Math.abs(position.positionAmt), direction, this.config.lossLimit);
      const activationPrice = calcTrailingActivationPrice(position.entryPrice, Math.abs(position.positionAmt), direction, this.config.trailingProfit);
      ...
      const currentTrailing = this.openOrders.find((o) => o.type === 'TRAILING_STOP_MARKET' && o.side === stopSide);
      ...
      // 步进式锁盈移动：在动态止盈生效前，盈利每增加一个 profitLockOffsetUsd 就上移/下移一次止损
      {
         ...
         const trailingActivate = Number.isFinite(trailingActivateFromOrder) ? trailingActivateFromOrder : activationPrice;

         // 判断动态止盈是否已生效：多头 price >= activate；空头 price <= activate
         const trailingActivated = direction === 'long' ? Number.isFinite(trailingActivate) && price >= trailingActivate - tick : Number.isFinite(trailingActivate) && price <= trailingActivate + tick;
         ...
         if (!trailingActivated && qtyAbs > 0 && stepUsd > 0) {
            ...
            if (basisProfit >= triggerUsd) {
               ...
               const rawTarget = direction === 'long' ? position.entryPrice + steps * stepPx : position.entryPrice - steps * stepPx;
               ...
               if (Number.isFinite(trailingActivate)) {
                  if (stopSide === 'SELL' && targetStop >= trailingActivate - tick) {
                     ...
                     const existingPrice = Number.isFinite(existingRaw) ? existingRaw : Number.NaN;
                     const canImprove = !Number.isFinite(existingPrice) || (stopSide === 'SELL' && targetStop >= existingPrice + tick);
                     if (!canImprove) {
                        // 直接跳过
                        // no-op
                     } else if (currentStop) {
                     ...
                     const existingRaw = Number(currentStop?.stopPrice);
                     const existingPrice = Number.isFinite(existingRaw) ? existingRaw : Number.NaN;
                     const canImprove = !Number.isFinite(existingPrice) || (stopSide === 'BUY' && targetStop <= existingPrice - tick);
                     if (!canImprove) {
                        // no-op
                     } else if (currentStop) {
                        await this.tryReplaceStop(stopSide, currentStop, targetStop, price);
                     ...
                     // 正常范围内，且必须与当前价方向不冲突
                     const validForSide = (stopSide === 'SELL' && targetStop <= price - tick) || (stopSide === 'BUY' && targetStop >= price + tick);
                     if (validForSide) {
                        if (!currentStop) {
                           await this.tryPlaceStopLoss(stopSide, targetStop, price);
                        } else {
                           const existingRaw = Number(currentStop.stopPrice);
                           const existingPrice = Number.isFinite(existingRaw) ? existingRaw : Number.NaN;
                           const improves = !Number.isFinite(existingPrice) || (stopSide === 'SELL' && targetStop >= existingPrice + tick) || (stopSide === 'BUY' && targetStop <= existingPrice - tick);
                           if (improves) {
                              await this.tryReplaceStop(stopSide, currentStop, targetStop, price);
                           }
                        }
      ...
      if (!currentTrailing && this.exchange.supportsTrailingStops()) {
         await this.tryPlaceTrailingStop(stopSide, Number(formatPriceToString(activationPrice, Math.max(0, Math.floor(Math.log10(1 / this.config.priceTick))))), Math.abs(position.positionAmt));
      }
      ...
      if (derivedLoss || snapshotLoss) {
         ...
         try {
            ...
            if (mark !== null && Number.isFinite(mark) && mark > 0 && Number.isFinite(closeSidePrice)) {
               ...
               if (pctDiff > limitPct) {
                  this.tradeLog.push('info', `市价平仓保护触发：closePx=${Number(closeSidePrice).toFixed(2)} mark=${mark.toFixed(2)} 偏离 ${(pctDiff * 100).toFixed(2)}% > ${(limitPct * 100).toFixed(2)}%`);
                  return { closed: false, pnl };
               }
            }
            await marketClose(this.exchange, this.config.symbol, this.openOrders, this.locks, this.timers, this.pending, direction === 'long' ? 'SELL' : 'BUY', Math.abs(position.positionAmt), (type, detail) => this.tradeLog.push(type, detail), {
               markPrice: getPosition(this.accountSnapshot, this.config.symbol).markPrice,
               expectedPrice: Number(direction === 'long' ? this.depthSnapshot?.bids?.[0]?.[0] : this.depthSnapshot?.asks?.[0]?.[0]) || null,
               maxPct: this.config.maxCloseSlippagePct,
            }, { qtyStep: this.config.qtyStep });
            result.closed = true;
   ...
   private async tryPlaceStopLoss(side: 'BUY' | 'SELL', stopPrice: number, lastPrice: number): Promise<void> {
      ...
      const tick = Math.max(1e-9, this.config.priceTick);
      const now = Date.now();
      if (this.lastStopAttempt.side === side && this.lastStopAttempt.price !== null && Math.abs(stopPrice - Number(this.lastStopAttempt.price)) < tick && now - this.lastStopAttempt.at < 5000) {
         // 5 秒内同向同价重复尝试，直接跳过
         return;
      }
      try {
   ...
   private async tryReplaceStop(side: 'BUY' | 'SELL', currentOrder: AsterOrder, nextStopPrice: number, lastPrice: number): Promise<void> {
   ...
   private async tryPlaceTrailingStop(side: 'BUY' | 'SELL', activationPrice: number, quantity: number): Promise<void> {
      if (!this.exchange.supportsTrailingStops()) {
         return;
      }
      try {
         await placeTrailingStopOrder(this.exchange, this.config.symbol, this.openOrders, this.locks, this.timers, this.pending, side, activationPrice, quantity, this.config.trailingCallbackRate, (type, detail) => this.tradeLog.push(type, detail), {
            markPrice: getPosition(this.accountSnapshot, this.config.symbol).markPrice,
            maxPct: this.config.maxCloseSlippagePct,
         }, { priceTick: this.config.priceTick, qtyStep: this.config.qtyStep });
      } catch (err) {
   ...
   private buildSnapshot(): TrendEngineSnapshot {
      ...
      const trend = price === null || sma30 === null ? '无信号' : price > sma30 ? '做多' : price < sma30 ? '做空' : '无信号';
   ...
   private trackPositionLifecycle(position: PositionSnapshot, referencePrice: number | null): void {
      ...
      const signChanged = prevExposure && currentExposure && Math.sign(prev.positionAmt) !== Math.sign(position.positionAmt);
      ...
      this.lastAccountPosition = { positionAmt: position.positionAmt, entryPrice: position.entryPrice, unrealizedProfit: position.unrealizedProfit, markPrice: position.markPrice };
   }

   private estimateRealizedPnl(position: PositionSnapshot, referencePrice: number | null): number {
      const fallbackPrice = referencePrice ?? this.getReferencePrice() ?? (this.lastPrice !== null && Number.isFinite(this.lastPrice) ? this.lastPrice : position.entryPrice);
