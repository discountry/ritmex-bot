...
export class GridEngine {
   ...
   private async closePosition(): Promise<void> {
      ...
      try {
         await placeMarketOrder(this.exchange, this.config.symbol, this.openOrders, this.locks, this.timers, this.pendings, side, amount, this.log, true, undefined, { qtyStep: this.config.qtyStep });
   ...
   private async syncGrid(price: number): Promise<void> {
      ...
      const orderMap = new Map<string, AsterOrder>(activeOrders.map((order) => [this.getOrderKey(order.side, this.normalizePrice(order.price)), order]));

      for (const order of activeOrders) {
         const key = this.getOrderKey(order.side, this.normalizePrice(order.price));
         if (desiredKeys.has(key)) { continue; }
         await safeCancelOrder(this.exchange, this.config.symbol, order, (orderId) => {
      ...
      for (const desiredOrder of desired) {
         ...
         try {
            const placed = await placeOrder(this.exchange, this.config.symbol, this.openOrders, this.locks, this.timers, this.pendings, desiredOrder.side, desiredOrder.price, desiredOrder.amount, this.log, desiredOrder.reduceOnly, undefined, {
   ...
   private computeDesiredOrders(price: number): DesiredGridOrder[] {
      ...
      const belowPrice = this.gridLevels.map((levelPrice, level) => ({ level, levelPrice })).filter(({ levelPrice }) => levelPrice < price - halfTick).sort((a, b) => b.levelPrice - a.levelPrice);
      const abovePrice = this.gridLevels.map((levelPrice, level) => ({ level, levelPrice })).filter(({ levelPrice }) => levelPrice > price + halfTick).sort((a, b) => a.levelPrice - b.levelPrice);
      ...
      for (const { level, levelPrice } of belowPrice) {
         ...
         desired.push({ level, side: 'BUY', price: this.formatPrice(levelPrice), amount, reduceOnly });
      }
      ...
      for (const { level, levelPrice } of abovePrice) {
         ...
         desired.push({ level, side: 'SELL', price: this.formatPrice(levelPrice), amount, reduceOnly });
      }
   ...
   private buildSnapshot(): GridEngineSnapshot {
      ...
      const openOrderKeys = new Set(this.openOrders.filter((order) => order.symbol === this.config.symbol && order.type === 'LIMIT').map((order) => this.getOrderKey(order.side, this.normalizePrice(order.price))));
      ...
      const gridLines: GridLineSnapshot[] = this.gridLevels.map((price, level) => {
         ...
         return { level, price, side, active, hasOrder, reduceOnly: desired?.reduceOnly ?? false };
      });
