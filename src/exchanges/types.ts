...
export type OrderType = 'LIMIT' | 'MARKET' | 'STOP_MARKET' | 'TRAILING_STOP_MARKET';
...
export interface GrvtBalancesResponse {
   result?: { total_unrealized_pnl?: string; positions?: GrvtPosition[] };
}
...
export interface GrvtTriggerMetadata {
   ...
   tpsl: { trigger_by: 'UNSPECIFIED' | 'INDEX' | 'LAST' | 'MID' | 'MARK'; trigger_price: string; close_position: boolean };
}
