...
export async function createClosePositionOrder(intent: ClosePositionIntent): Promise<AsterOrder> {
   const params: CreateOrderParams = applyCommonFields({ symbol: intent.symbol, side: intent.side, type: 'MARKET', quantity: intent.quantity, reduceOnly: 'true', closePosition: toStringBoolean(intent.closePosition ?? true) }, intent);
   return intent.adapter.createOrder(params);
}
