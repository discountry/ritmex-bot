...
import type { IApiCandlestickResponse, IApiCreateOrderResponse, IApiOpenOrdersResponse, IApiOrderbookLevelsResponse, IApiPositionsResponse, IApiSubAccountSummaryResponse, IApiTickerResponse, IOrder } from '@grvt/client/interfaces';
...
import { EStream, type TWSRequest, WS } from '@grvt/client/ws';
...
const ENVIRONMENT_HOSTS = {
   prod: { trades: 'https://trades.grvt.io', market: 'https://market-data.grvt.io', edge: 'https://edge.grvt.io' },
   testnet: { trades: 'https://trades.testnet.grvt.io', market: 'https://market-data.testnet.grvt.io', edge: 'https://edge.testnet.grvt.io' },
   staging: { trades: 'https://trades.staging.gravitymarkets.io', market: 'https://market-data.staging.gravitymarkets.io', edge: 'https://edge.staging.gravitymarkets.io' },
   dev: { trades: 'https://trades.dev.gravitymarkets.io', market: 'https://market-data.dev.gravitymarkets.io', edge: 'https://edge.dev.gravitymarkets.io' },
} as const;
...
const CHAIN_IDS: Record<keyof typeof ENVIRONMENT_HOSTS, number> = { prod: 325, testnet: 326, staging: 327, dev: 327 };
...
const EIP712_ORDER_TYPES = {
   Order: [{ name: 'subAccountID', type: 'uint64' }, { name: 'isMarket', type: 'bool' }, { name: 'timeInForce', type: 'uint8' }, { name: 'postOnly', type: 'bool' }, { name: 'reduceOnly', type: 'bool' }, { name: 'legs', type: 'OrderLeg[]' }, {
      name: 'nonce',
      type: 'uint32',
...
const EIP712_DOMAIN_FIELDS = [{ name: 'name', type: 'string' }, { name: 'version', type: 'string' }, { name: 'chainId', type: 'uint256' }] as const;
...
export type GrvtSignatureProvider = (context: GrvtSignContext) => Promise<GrvtSignature>;
...
export interface GrvtGatewayOptions {
   ...
   pollIntervals?: Partial<{ account: number; orders: number; depth: number; ticker: number; klines: number }>;
...
export class GrvtGateway {
   ...
   private readonly pollIntervals: { account: number; orders: number; depth: number; ticker: number; klines: number };
   ...
   constructor(options: GrvtGatewayOptions) {
      ...
      if (options.cookie && options.accountId) {
         ...
         this.sessionInfo = { cookie: normalizedCookie, accountId: options.accountId };
         this.updateHeaders();
      }
   ...
   async ensureInitialized(klineInterval: string): Promise<void> {
      ...
      await Promise.all([this.refreshAccountSnapshot(), this.refreshOpenOrders(), this.refreshPositions(), this.refreshDepth(), this.refreshTicker(), this.refreshKlines()]);
      this.startPolling();
      this.startWebsocket();
   ...
   async createOrder(params: CreateOrderParams): Promise<AsterOrder> {
      ...
      const { order: unsignedOrder, signing } = buildUnsignedOrder({ params, instrument: this.instrument, subAccountId: this.subAccountId });
      ...
      const signature = this.signatureProvider ? await this.signatureProvider(signContext) : await this.signWithPrivateKey(signContext);
      ...
      try {
         ...
         const payload = { code: (error as any)?.response?.data?.code, message: (error as any)?.response?.data?.message };
   ...
   async cancelOrders(params: { symbol: string; orderIdList: Array<number | string> }): Promise<void> {
      await Promise.all(params.orderIdList.map((orderId) =>
         this.tdg.cancelOrder({ sub_account_id: this.subAccountId, order_id: String(orderId) }).catch((error) => {
      ...
      try {
         ...
         const ws = new WS({
            ...
            reconnectStrategy: (retries: number) => {
               ...
               const base = 1000 * 2 ** Math.min(8, attempt);
         ...
         ws.onConnect(() => {
            ...
            const subs: TWSRequest[] = [
               { stream: EStream.POSITION, params: { sub_account_id: this.subAccountId, instrument: this.instrument }, onData: () => this.schedulePositionsRefresh(150) } as any,
               { stream: EStream.ORDER, params: { sub_account_id: this.subAccountId, instrument: this.instrument }, onData: () => this.schedulePositionsRefresh(100) } as any,
               { stream: EStream.STATE, params: { sub_account_id: this.subAccountId, instrument: this.instrument }, onData: () => this.schedulePositionsRefresh(100) } as any,
            ];
            ws.subscribes(...subs);
         });
         ws.onClose(() => {
   ...
   private async refreshDepth(): Promise<void> {
      try {
         const response = await this.mdg.orderBook({ instrument: this.instrument, depth: 50 });
   ...
   private async refreshKlines(): Promise<void> {
      const response = await this.mdg.candlestick({ instrument: this.instrument, interval: this.klineInterval, type: ECandlestickType.TRADE, limit: 500 });
   ...
   private async authenticate(): Promise<void> {
      ...
      try {
         const response = await axios.post(`${this.hosts.edge}/auth/api_key/login`, { api_key: this.apiKey }, { headers: { 'Content-Type': 'application/json', Cookie: 'rm=true;' }, timeout: 5000 });
         ...
         const accountId = Array.isArray(accountIdHeader) ? accountIdHeader[0]?.toString().trim() : accountIdHeader?.toString().trim();
         if (!cookieInfo?.cookie || !accountId) {
            throw new Error('GRVT authentication response missing session cookie or account id');
         }
         this.sessionInfo = { cookie: cookieInfo.cookie, accountId, expiresAt: cookieInfo.expiresAt };
         this.updateHeaders();
      } catch (error) {
   ...
   private updateHeaders(): void {
      ...
      this.headers = { Cookie: this.sessionInfo.cookie, 'X-Grvt-Account-Id': this.sessionInfo.accountId };
   }
   ...
   private async loadInstrumentInfo(): Promise<void> {
      ...
      try {
         ...
         this.instrumentInfo = { instrument: this.instrument, instrumentHash: normalizeHex(instrumentHashRaw), baseDecimals, quoteDecimals };
      } catch (error) {
   ...
   private async signWithPrivateKey(context: GrvtSignContext): Promise<GrvtSignature> {
      ...
      const types: EIP712Types = { EIP712Domain: EIP712_DOMAIN_FIELDS.map((field) => ({ ...field })), Order: EIP712_ORDER_TYPES.Order.map((field) => ({ ...field })), OrderLeg: EIP712_ORDER_TYPES.OrderLeg.map((field) => ({ ...field })) };
      const message = {
         ...
         legs: [{ assetID: BigInt(normalizeHex(context.instrument.instrumentHash)), contractSize, limitPrice, isBuyingContract: context.side === 'BUY' }],
         nonce: BigInt(context.nonce),
         expiration: context.expirationNs,
      };
      const domain = { name: 'GRVT Exchange', version: '0', chainId: BigInt(context.chainId) };
      ...
      return { signer, r, s, v, expiration: context.expirationNs.toString(), nonce: context.nonce };
   }
}
...
function resolveHosts(env: BaseEnvironment, override?: GrvtHostsOverride): HostsConfig {
   ...
   return { trades: override?.trades ?? base.trades, market: override?.market ?? base.market, edge: override?.edge ?? base.edge };
}
...
function parseSetCookieHeader(header?: string | string[]): { cookie: string; expiresAt?: number } | null {
   ...
   for (const entry of entries) {
      ...
      return { cookie: cookieSegment, expiresAt: Number.isFinite(expiresAt) ? expiresAt : undefined };
   }
   return null;
}
...
function cloneDepth(depth: AsterDepth): AsterDepth {
   return { ...depth, bids: depth.bids.map((level) => [...level] as [string, string]), asks: depth.asks.map((level) => [...level] as [string, string]) };
}
...
function nsToMs(input: string | number | undefined): number {
   if (!input || input === null) { return Date.now(); }
...
function mapAccountSnapshot(response: IApiSubAccountSummaryResponse, symbol: string, instrument: string): AsterAccountSnapshot {
   const result = response.result;
   if (!result) {
      return emptyAccount(symbol);
   }
   const updateTime = nsToMs(result.event_time);
   const assets = (result.spot_balances ?? []).map((balance) => ({ asset: balance.currency ?? symbol.slice(-4), walletBalance: balance.balance ?? '0', availableBalance: balance.balance ?? '0', updateTime }));
   return {
      canTrade: true,
      canDeposit: true,
      canWithdraw: true,
...
function mapPositions(response: IApiPositionsResponse, symbol: string, instrument: string): AsterAccountPosition[] {
   return (response.result ?? []).filter((entry) => !entry.instrument || entry.instrument === instrument).map((entry) => ({
      symbol,
...
function mapDepth(response: IApiOrderbookLevelsResponse, symbol: string): AsterDepth | null {
   ...
   const toLevel = (entries: Array<{ price?: string; size?: string }> | undefined) => (entries ?? []).map((entry) => [entry.price ?? '0', entry.size ?? '0'] as [string, string]);
   return { lastUpdateId: Number(result.event_time ?? Date.now()), eventTime: nsToMs(result.event_time), symbol, bids: toLevel(result.bids as any), asks: toLevel(result.asks as any) };
}
...
function mapCreateOrderResponse(response: IApiCreateOrderResponse, symbol: string): AsterOrder {
   ...
   if (!order) {
      return { orderId: cryptoRandomId(), clientOrderId: '', symbol, side: 'BUY', type: 'LIMIT', status: 'NEW', price: '0', origQty: '0', executedQty: '0', stopPrice: '0', time: Date.now(), updateTime: Date.now(), reduceOnly: false, closePosition: false };
   }
...
function toApiOrderPayload(order: GrvtSignedOrder): IOrder {
   ...
   return { ...order, time_in_force: toApiTimeInForce(order.time_in_force), metadata };
}
...
function toApiOrderMetadata(metadata: GrvtOrderMetadataInput): IOrder['metadata'] {
   ...
   return {
      ...
      trigger: trigger ? { trigger_type: toApiTriggerType(trigger.trigger_type), tpsl: { trigger_by: toApiTriggerBy(trigger.tpsl.trigger_by), trigger_price: trigger.tpsl.trigger_price, close_position: trigger.tpsl.close_position } } : undefined,
   };
}
...
function buildUnsignedOrder(
   params: { params: CreateOrderParams; instrument: string; subAccountId: string },
): { order: GrvtUnsignedOrder; signing: { quantity: number; price?: number; side: OrderSide; isMarket: boolean; timeInForce: string; postOnly: boolean; reduceOnly: boolean } } {
   ...
   const metadata = { client_order_id: generateClientOrderId(), ...(trigger ? { trigger } : {}) };
   ...
   const order: GrvtUnsignedOrder = {
      ...
      legs: [{ instrument, size: toDecimalString(quantity), limit_price: isMarketOrder ? undefined : toDecimalString(priceValue), is_buying_asset: orderParams.side === 'BUY' }],
      metadata,
   };

   return { order, signing: { quantity, price: priceValue === null ? undefined : Number(priceValue), side: orderParams.side, isMarket: isMarketOrder, timeInForce, postOnly, reduceOnly } };
}
...
function buildTriggerMetadata(params: CreateOrderParams): GrvtUnsignedOrder['metadata']['trigger'] | undefined {
   if (params.type === 'STOP_MARKET') {
      ...
      return { trigger_type: triggerType, tpsl: { trigger_by: DEFAULT_MARK_PRICE_TRIGGER, trigger_price: toDecimalString(stopPrice), close_position: normalizeBoolean(params.closePosition) } };
   }
   return undefined;
}
...
function emptyAccount(symbol: string): AsterAccountSnapshot {
   return {
      ...
      positions: [],
      assets: [{ asset: symbol.slice(-4), walletBalance: '0', availableBalance: '0', updateTime: Date.now() }],
