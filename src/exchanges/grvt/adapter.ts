...
import { type GrvtEnvironment, GrvtGateway, type GrvtGatewayOptions, type GrvtHostsOverride, type GrvtSignatureProvider } from './gateway';
...
export class GrvtExchangeAdapter implements ExchangeAdapter {
   ...
   private ensureInitialized(context?: string, interval?: string): Promise<void> {
      ...
      if (!this.initPromise) {
         ...
         this.initPromise = this.gateway.ensureInitialized(this.klineInterval).then((value) => {
            this.clearRetry();
            return value;
         }).catch((error) => {
...
function loadSignatureProviderFromEnv(logger?: GrvtGatewayOptions['logger']): GrvtSignatureProvider | undefined {
   const signerModule = process.env.GRVT_SIGNER_PATH;
   if (!signerModule) { return undefined; }
   try {
      const require = createRequire(import.meta.url);
      const resolved = signerModule.startsWith('.') || signerModule.startsWith('/') ? path.resolve(process.cwd(), signerModule) : signerModule;
