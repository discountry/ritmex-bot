...
type PendingResolver = { resolve: (value: any) => void; reject: (error: Error) => void };
...
export class LighterSigner {
   ...
   constructor(config: LighterSignerConfig) {
      this.accountIndex = typeof config.accountIndex === 'number' ? BigInt(Math.trunc(config.accountIndex)) : config.accountIndex;
      ...
      this.ready = (async () => {
         for (const [index, key] of entries) {
            await bridge.call('create_client', { apiKeyIndex: Number(index), privateKey: key, baseUrl, chainId, accountIndex: accountIndexStr });
         }
      })();
   }
   ...
   async signCreateOrder(params: CreateOrderSignParams): Promise<SignedTxPayload> {
      ...
      return { txType: 14, txInfo, txHash, signature };
   }
   ...
   async signCancelOrder(params: CancelOrderSignParams): Promise<SignedTxPayload> {
      ...
      const result = await this.bridge.call('sign_cancel_order', { apiKeyIndex, marketIndex: params.marketIndex, orderIndex: params.orderIndex.toString(), nonce: params.nonce.toString() });
      ...
      return { txType: 15, txInfo, signature };
   }
   ...
   async signCancelAll(params: CancelAllSignParams): Promise<SignedTxPayload> {
      ...
      const result = await this.bridge.call('sign_cancel_all', { apiKeyIndex, timeInForce: params.timeInForce, scheduledTime: params.scheduledTime.toString(), nonce: params.nonce.toString() });
      ...
      return { txType: 16, txInfo, signature };
   }

   async createAuthToken(deadlineMs: number, apiKeyIndex?: number): Promise<string> {
      await this.ensureReady();
      const index = apiKeyIndex ?? this.defaultKeyIndex;
      const result = await this.bridge.call('create_auth_token', { apiKeyIndex: index, deadlineMs: Math.floor(deadlineMs / 1000) });
