...
import { type CreateOrderSignParams, LighterSigner } from './signer';
import type { LighterAccountDetails, LighterKline, LighterMarketStats, LighterOrder, LighterOrderBookLevel, LighterOrderBookSnapshot, LighterPosition } from './types';
...
export class LighterGateway {
   ...
   constructor(options: LighterGatewayOptions) {
      ...
      this.signer = new LighterSigner({ accountIndex: options.accountIndex, chainId: options.chainId ?? (this.environment === 'mainnet' ? 304 : 300), apiKeys: options.apiKeys, baseUrl: host });
      this.apiKeyIndices = options.apiKeyIndices ?? Object.keys(options.apiKeys).map(Number);
      this.nonceManager = new HttpNonceManager({ accountIndex: options.accountIndex, apiKeyIndices: this.apiKeyIndices, http: this.http });
   ...
   async createOrder(params: CreateOrderParams): Promise<AsterOrder> {
      ...
      try {
         const signed = await this.signer.signCreateOrder({ ...signParams, apiKeyIndex, nonce });
         ...
         const response = await this.http.sendTransaction(signed.txType, signed.txInfo, { authToken: auth, priceProtection: false });
         if (process.env.LIGHTER_DEBUG === '1' || process.env.LIGHTER_DEBUG === 'true') {
            this.logger('createOrder.sendTx.response', response);
         }
         return lighterOrderToAster(
            this.displaySymbol,
            {
   ...
   private async refreshAccountSnapshot(): Promise<void> {
      try {
         ...
         if (this.l1Address) {
            details = await this.http.getAccountDetails(Number(this.signer.accountIndex), auth, { by: 'l1_address', value: this.l1Address });
         }
         if (!details) {
            details = await this.http.getAccountDetails(Number(this.signer.accountIndex), auth, { by: 'index', value: Number(this.signer.accountIndex) });
         }
         if (details) {
            ...
            this.accountDetails = { account_index: Number(this.signer.accountIndex), status: 1, collateral: '0', available_balance: '0' } as LighterAccountDetails;
            this.positions = [];
            this.emitAccount();
         }
      } catch (error) {
   ...
   private async subscribeChannels(): Promise<void> {
      ...
      ws.send(JSON.stringify({ type: 'subscribe', channel: `account_market/${Number(marketId)}/${Number(this.signer.accountIndex)}`, auth }));
      ws.send(JSON.stringify({ type: 'subscribe', channel: `account_all_orders/${Number(this.signer.accountIndex)}`, auth }));
   }
   ...
   private async refreshKlines(interval: string): Promise<void> {
      ...
      const raw = await this.http.getCandlesticks({ marketId, resolution: interval, countBack: count, endTimestamp: endTs, startTimestamp: startTs, setTimestampToEnd: true });
   ...
   private mapCreateOrderParams(params: CreateOrderParams): Omit<CreateOrderSignParams, 'nonce'> & { baseAmountScaledString: string; priceScaledString: string; triggerPriceScaledString: string; clientOrderIndex: bigint } {
      ...
      if (params.stopPrice !== null) {
         triggerPriceScaled = decimalToScaled(params.stopPrice ?? 0, this.priceDecimals);
      }
      ...
      const orderExpiry = isImmediate ? BigInt(IMMEDIATE_OR_CANCEL_EXPIRY_PLACEHOLDER) : BigInt(Date.now() + TWENTY_EIGHT_DAYS_MS);
...
function sortAndTrimLevels(levels: LighterOrderBookLevel[] | undefined, side: 'bid' | 'ask', limit: number = 200): LighterOrderBookLevel[] {
