...
export type LighterOrderType = 'limit' | 'market' | 'stop_loss' | 'stop_loss_limit' | 'take_profit' | 'take_profit_limit' | string;
...
export interface LighterSignerBinding {
   createClient(url: string, apiPrivateKey: string, chainId: number, apiKeyIndex: number, accountIndex: number): string | null;
   switchApiKey(apiKeyIndex: number): string | null;
   createAuthToken(deadlineSeconds: number): StrOrErr;
   signCreateOrder(params: { marketIndex: number; clientOrderIndex: number; baseAmount: bigint; price: bigint; isAsk: number; orderType: number; timeInForce: number; reduceOnly: number; triggerPrice: bigint; orderExpiry: bigint; nonce: bigint }): StrOrErr;
   signCancelOrder(params: { marketIndex: number; orderIndex: bigint; nonce: bigint }): StrOrErr;
   signCancelAllOrders(params: { timeInForce: number; time: bigint; nonce: bigint }): StrOrErr;
}
