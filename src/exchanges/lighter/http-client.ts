...
import type { LighterAccountDetails, LighterKline, LighterMarketStats, LighterOrderBookMetadata } from './types';
...
export class LighterHttpClient {
   ...
   async getAccountDetails(accountIndex: number, authToken?: string, options: { by?: 'index' | 'l1_address'; value?: string | number } = {}): Promise<LighterAccountDetails | null> {
      ...
      const response = await this.get<AccountResponse>('/api/v1/account', { query, headers: authToken ? { Authorization: authToken } : undefined, tolerateNotFound: true });
      return response.account ?? null;
   }

   async getCandlesticks(params: { marketId: number; resolution: string; countBack: number; startTimestamp: number; endTimestamp: number; setTimestampToEnd?: boolean }): Promise<LighterKline[]> {
      const response = await this.get<CandlesticksResponse>('/api/v1/candlesticks', {
         query: { market_id: params.marketId, resolution: params.resolution, count_back: params.countBack, start_timestamp: params.startTimestamp, end_timestamp: params.endTimestamp, set_timestamp_to_end: params.setTimestampToEnd ?? true },
      });

      return (response.candlesticks ?? []).map((entry) => ({
         start_timestamp: entry.start_timestamp ?? entry.timestamp ?? 0,
         end_timestamp: entry.end_timestamp ?? (entry.start_timestamp ?? entry.timestamp ?? 0) + 1,
   ...
   async getNextNonce(accountIndex: number, apiKeyIndex: number): Promise<bigint> {
      const response = await this.get<NextNonceResponse>('/api/v1/nextNonce', { query: { account_index: accountIndex, api_key_index: apiKeyIndex } });
   ...
   async sendTransaction(txType: number, txInfo: string, options: { priceProtection?: boolean; authToken?: string } = {}): Promise<SendTxResponse> {
   ...
   private async get<T extends ApiResponseBase>(path: string, options: { query?: Record<string, unknown>; headers?: Record<string, string | undefined>; tolerateNotFound?: boolean } = {}): Promise<T> {
      ...
      const response = await this.fetcher(requestUrl, { method: 'GET', headers: { 'Accept': 'application/json', ...this.cleanHeaders(options.headers) } });
   ...
   private async post<T extends ApiResponseBase>(path: string, body: unknown, authToken?: string): Promise<T> {
      ...
      const response = await this.fetcher(requestUrl, { method: 'POST', headers: { 'Content-Type': 'application/json', Accept: 'application/json', ...(authToken ? { Authorization: authToken } : {}) }, body: JSON.stringify(body) });
      return this.parseResponse<T>(response, requestUrl);
   }

   private async postForm<T extends ApiResponseBase>(path: string, form: FormData, authToken?: string): Promise<T> {
      const requestUrl = new URL(path, `${this.baseUrl}`).toString();
      const response = await this.fetcher(requestUrl, { method: 'POST', headers: { Accept: 'application/json', ...(authToken ? { Authorization: authToken } : {}) }, body: form as any });
      return this.parseResponse<T>(response, requestUrl);
   }
