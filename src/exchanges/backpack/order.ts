...
export async function createLimitOrder(intent: LimitOrderIntent): Promise<AsterOrder> {
   const params: CreateOrderParams = applyCommonFields({ symbol: intent.symbol, side: intent.side, type: 'LIMIT', quantity: intent.quantity, price: intent.price, timeInForce: intent.timeInForce ?? 'GTX' }, intent);
...
export async function createMarketOrder(intent: MarketOrderIntent): Promise<AsterOrder> {
   const params: CreateOrderParams = applyCommonFields({ symbol: intent.symbol, side: intent.side, type: 'MARKET', quantity: intent.quantity }, intent);
   return intent.adapter.createOrder(params);
}

export async function createStopOrder(intent: StopOrderIntent): Promise<AsterOrder> {
   const params: CreateOrderParams = applyCommonFields({ symbol: intent.symbol, side: intent.side, type: 'STOP_MARKET', quantity: intent.quantity, stopPrice: intent.stopPrice, timeInForce: intent.timeInForce ?? 'GTC' }, intent);
   return intent.adapter.createOrder(params);
}
...
export async function createClosePositionOrder(intent: ClosePositionIntent): Promise<AsterOrder> {
   const params: CreateOrderParams = applyCommonFields({ symbol: intent.symbol, side: intent.side, type: 'MARKET', quantity: intent.quantity, reduceOnly: 'true' }, intent);
   return intent.adapter.createOrder(params);
}
