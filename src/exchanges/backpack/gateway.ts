import { hashes as edHashes, sign, utils as edUtils } from '@noble/ed25519';
...
const WebSocketCtor: typeof globalThis.WebSocket = typeof globalThis.WebSocket !== 'undefined' ? globalThis.WebSocket : ((NodeWebSocket as unknown) as typeof globalThis.WebSocket);
...
const TRIGGER_TOPICS = { orders: (marketId: string) => `account.orderUpdate.${marketId}`, positions: (marketId: string) => `account.positionUpdate.${marketId}` };
...
export class BackpackGateway {
   ...
   private async doInitialize(symbol?: string): Promise<void> {
      try {
         ...
         if (process.env.BACKPACK_DEBUG === '1') {
            console.debug('[BackpackGateway] marketInfo', { userSymbol: this.symbol, ccxtSymbol: this.marketSymbol, marketId: this.marketId });
         }
   ...
   private findMarket(requested: string): any | null {
      const normalize = (value: string | undefined | null): string => (value ?? '').toUpperCase().replace(/[^A-Z0-9]/g, '');
   ...
   private startOrderPolling(): void {
      ...
      const poll = async () => {
         try {
            const [openOrders, allOrders] = await Promise.all([this.exchange.fetchOpenOrders(this.marketSymbol), this.exchange.fetchOrders(this.marketSymbol, undefined, 200, {})]);
   ...
   private startKlinePolling(interval: string): void {
      ...
      const poll = async () => {
         try {
            ...
            const klines = (ohlcv as number[][]).filter((row) => Array.isArray(row) && row.length >= 6).map((row) => this.mapOHLCVToKline(row as [number, number, number, number, number, number], interval));
   ...
   private mapBalanceToAccountSnapshotWithPositions(balance: Balances, rawPositions: any[]): AsterAccountSnapshot {
      ...
      const snapshot: AsterAccountSnapshot = { canTrade: true, canDeposit: true, canWithdraw: true, updateTime: now, totalWalletBalance, totalUnrealizedProfit: totalUnrealized, positions, assets, availableBalance, maxWithdrawAmount: availableBalance };
   ...
   private normalizePositions(rawPositions: any[], now: number): AsterAccountSnapshot['positions'] {
      ...
      for (const raw of rawPositions) {
         ...
         positions.push({ symbol: this.symbol, positionAmt: positionAmt.toString(), entryPrice, unrealizedProfit: unrealized, positionSide: 'BOTH', updateTime: now, markPrice, liquidationPrice, leverage, marginType: 'CROSSED' });
      }
      return positions;
   }
   ...
   private mapOrderBookToDepth(orderbook: CcxtOrderBook): AsterDepth {
      return {
         ...
         bids: (orderbook.bids ?? []).filter((row) => row && row.length >= 2).map(([price, amount]) => [String(price ?? 0), String(amount ?? 0)]),
         asks: (orderbook.asks ?? []).filter((row) => row && row.length >= 2).map(([price, amount]) => [String(price ?? 0), String(amount ?? 0)]),
   ...
   private mapOHLCVToKline(candle: [number, number, number, number, number, number], interval: string): AsterKline {
      ...
      return { openTime, closeTime: openTime + this.intervalToMs(interval), open: open.toString(), high: high.toString(), low: low.toString(), close: close.toString(), volume: volume.toString(), numberOfTrades: 0 };
   }
   ...
   private mapRestOrder(order: CcxtOrder): AsterOrder {
      ...
      if (order.triggerPrice !== null || info.triggerPrice !== null || info.stopLossTriggerPrice !== null || info.Y !== null || info.y !== null) {
         type = 'STOP_MARKET';
      }
   ...
   private mapWsPosition(data: Record<string, unknown>): AsterAccountPosition | null {
      ...
      return { symbol: this.symbol, positionAmt, entryPrice, unrealizedProfit: unrealized, positionSide: 'BOTH', updateTime: Number.isFinite(updateTime) ? Math.floor(updateTime / 1000) : Date.now(), markPrice, leverage, marginType: 'CROSSED' };
   }

   private mergeWsPosition(position: AsterAccountPosition): void {
      const snapshot: AsterAccountSnapshot = this.lastBalanceSnapshot
         ? { ...this.lastBalanceSnapshot, positions: this.lastBalanceSnapshot.positions ? [...this.lastBalanceSnapshot.positions] : [] }
         : { canTrade: true, canDeposit: true, canWithdraw: true, updateTime: Date.now(), totalWalletBalance: '0', totalUnrealizedProfit: '0', positions: [], assets: [], availableBalance: '0', maxWithdrawAmount: '0' };
   ...
   private connectPrivateSocket(): void {
      ...
      if ('addEventListener' in socket && typeof socket.addEventListener === 'function') {
         ...
         const off = typeof nodeSocket.off === 'function' ? (event: string, handler: (...args: any[]) => void) => nodeSocket.off(event, handler) : (event: string, handler: (...args: any[]) => void) => nodeSocket.removeListener(event, handler);
   ...
   private handleWsMessage = (event: any): void => {
      try {
         ...
         const raw = typeof payload === 'string' ? payload : Buffer.isBuffer(payload) ? payload.toString('utf8') : payload?.toString?.() ?? '';
         if (!raw) { return; }
         const message = JSON.parse(raw) as Record<string, unknown>;
         const stream = String(message.stream ?? '');
         const data = message.data as Record<string, unknown> | undefined;
         if (process.env.BACKPACK_DEBUG === '1') {
            console.debug('[BackpackGateway] wsMessage', { stream, data, result: message.result, symbol: this.symbol, marketId: this.marketId });
         }
         if (!data) { return; }
   ...
   private async sendWsSubscription(topics: string[], method: 'SUBSCRIBE' | 'UNSUBSCRIBE'): Promise<void> {
      ...
      try {
         ...
         const message = { method, params: topics, signature: [this.apiKey, signature, timestamp, this.wsWindow] };
         if (process.env.BACKPACK_DEBUG === '1') {
   ...
   private normalizeStatus(status?: string): string {
      ...
      return (ORDER_STATUS_MAP[key] ?? status.replace(/([a-z0-9])([A-Z])/g, '$1_$2').replace(/\s+/g, '_').toUpperCase());
   }
