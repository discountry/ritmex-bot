...
export class BackpackGateway {
   ...
   constructor(options: BackpackGatewayOptions) {
      ...
      this.exchange = new (ccxt as any).backpack({ apiKey: options.apiKey, secret: options.apiSecret, password: options.password, subaccount: options.subaccount, sandbox: options.sandbox ?? false, enableRateLimit: true, timeout: 30000 });
   }
   ...
   private resolveMarketSymbol(requested: string): string | null {
      ...
      const normalizeUsdAlias = (v: string) => {
         ...
         return up.replace(/USDT(?=(?:[_-]?PERP)?$)/, 'USDC').replace(/USD(?=(?:[_-]?PERP)?$)/, 'USDC');
      };
   ...
   private normalizeTimeframe(interval: string): string {
      const timeframeMap: Record<string, string> = { '1m': '1m', '5m': '5m', '15m': '15m', '1h': '1h', '4h': '4h', '1d': '1d' };
   ...
   private startKlinePolling(interval: string): void {
      ...
      const poll = async () => {
         try {
            ...
            const klines = (ohlcv as number[][]).filter((c) => Array.isArray(c) && c.length >= 6).map((c) => this.mapOHLCVToKline([c[0], c[1], c[2], c[3], c[4], c[5]] as [number, number, number, number, number, number], interval));
   ...
   private getIntervalMs(interval: string): number {
      const intervalMap: Record<string, number> = { '1m': 60 * 1000, '5m': 5 * 60 * 1000, '15m': 15 * 60 * 1000, '1h': 60 * 60 * 1000, '4h': 4 * 60 * 60 * 1000, '1d': 24 * 60 * 60 * 1000 };
