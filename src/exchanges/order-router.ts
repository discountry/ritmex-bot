...
import type { BaseOrderIntent, ClosePositionIntent, LimitOrderIntent, MarketOrderIntent, StopOrderIntent, TrailingStopOrderIntent } from './order-schema';
...
const handlerMap: Record<ExchangeKey, ExchangeOrderHandlers> = {
   aster: { limit: asterOrders.createLimitOrder, market: asterOrders.createMarketOrder, stop: asterOrders.createStopOrder, trailingStop: asterOrders.createTrailingStopOrder, close: asterOrders.createClosePositionOrder },
   backpack: { limit: backpackOrders.createLimitOrder, market: backpackOrders.createMarketOrder, stop: backpackOrders.createStopOrder, trailingStop: backpackOrders.createTrailingStopOrder, close: backpackOrders.createClosePositionOrder },
   grvt: { limit: grvtOrders.createLimitOrder, market: grvtOrders.createMarketOrder, stop: grvtOrders.createStopOrder, trailingStop: grvtOrders.createTrailingStopOrder, close: grvtOrders.createClosePositionOrder },
   lighter: { limit: lighterOrders.createLimitOrder, market: lighterOrders.createMarketOrder, stop: lighterOrders.createStopOrder, trailingStop: lighterOrders.createTrailingStopOrder, close: lighterOrders.createClosePositionOrder },
   paradex: { limit: paradexOrders.createLimitOrder, market: paradexOrders.createMarketOrder, stop: paradexOrders.createStopOrder, trailingStop: paradexOrders.createTrailingStopOrder, close: paradexOrders.createClosePositionOrder },
};
...
function resolveExchangeKey(adapter: ExchangeAdapter): ExchangeKey {
   ...
   throw new Error(`Unsupported exchange for order routing: ${candidates.filter(Boolean).join(', ') || 'unknown'}`);
}
