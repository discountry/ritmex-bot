...
export async function createStopOrder(intent: StopOrderIntent): Promise<AsterOrder> {
   const params: CreateOrderParams = applyCommonFields(
      { symbol: intent.symbol, side: intent.side, type: 'STOP_MARKET', quantity: intent.quantity, stopPrice: intent.stopPrice, timeInForce: intent.timeInForce ?? 'GTC', triggerType: intent.triggerType },
      intent,
   );
   return intent.adapter.createOrder(params);
}
