...
export class ParadexExchangeAdapter implements ExchangeAdapter {
   ...
   constructor(credentials: ParadexCredentials = {}) {
      ...
      this.gateway = new ParadexGateway({ symbol, displaySymbol: symbol, privateKey, walletAddress, sandbox, pollIntervals: credentials.pollIntervals, watchReconnectDelayMs, usePro, logger: (context, error) => this.logError(context, error) });
   ...
   private logError(context: string, error: unknown): void {
      ...
      const criticalContexts = ['initialize', 'accountPoll', 'watchBalanceLoop', 'orderPoll', 'orderPollOpen', 'orderPollClosed'];
      if (criticalContexts.some((prefix) => context.startsWith(prefix)) || process.env.PARADEX_DEBUG === '1' || process.env.PARADEX_DEBUG === 'true') {
