...
export async function createMarketOrder(intent: MarketOrderIntent): Promise<AsterOrder> {
   const params: CreateOrderParams = applyCommonFields({ symbol: intent.symbol, side: intent.side, type: 'MARKET', quantity: intent.quantity, timeInForce: intent.timeInForce }, intent);
   return intent.adapter.createOrder(params);
}
