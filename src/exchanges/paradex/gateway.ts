import ccxt, { type Balances, type OHLCV as CcxtOhlcv, type Order as CcxtOrder, type OrderBook as CcxtOrderBook, type Ticker as CcxtTicker } from 'ccxt';
import { createRequire } from 'node:module';
import { extractMessage } from '../../utils/errors';
import type { AccountListener, DepthListener, KlineListener, OrderListener, TickerListener } from '../adapter';
import type { AsterAccountAsset, AsterAccountSnapshot, AsterDepth, AsterKline, AsterOrder, AsterTicker, CreateOrderParams, OrderType } from '../types';
...
export interface ParadexGatewayOptions {
   ...
   paradexAccount?: { privateKey: string; publicKey: string; address: string };
   sandbox?: boolean;
   logger?: (context: string, error: unknown) => void;
   pollIntervals?: { account?: number; orders?: number; depth?: number; ticker?: number; klines?: number };
   usePro?: boolean;
...
type ParadexPollingConfig = { account: number; orders: number; depth: number; ticker: number; klines: number };
...
export class ParadexGateway {
   ...
   constructor(options: ParadexGatewayOptions) {
      ...
      const exchangeOptions: Record<string, unknown> = { enableRateLimit: true, timeout: 30000, sandboxMode: options.sandbox ?? false };
   ...
   async ensureInitialized(symbol?: string): Promise<void> {
      ...
      this.initPromise = this.doInitialize(symbol).then((value) => {
   ...
   private async runKlineProLoop(interval: string, control: ProLoopControl): Promise<void> {
      ...
      while (!this.destroyed && control.running) {
         try {
            ...
            const klines = candles.filter((candle) => Array.isArray(candle) && candle.length >= 6).map((candle) => this.mapOHLCVToKline(candle as CcxtOhlcv, interval));
   ...
   private startOrderPolling(): void {
      ...
      const poll = async () => {
         try {
            ...
            try {
               closedOrders = (await this.exchange.fetchClosedOrders(this.marketSymbol, undefined, 50)) as CcxtOrder[];
            } catch (error) {
   ...
   private startKlinePolling(interval: string): void {
      ...
      const poll = async () => {
         try {
            ...
            const klines = ohlcv.filter((candle) => Array.isArray(candle) && candle.length >= 6).map((candle) => this.mapOHLCVToKline(candle as CcxtOhlcv, interval));
            for (const listener of this.klineListeners.get(interval) ?? []) {
               listener(klines);
            }
         } catch (error) {
   ...
   private mapBalanceToAccountSnapshot(balance: Balances): AsterAccountSnapshot {
      ...
      this.logger('balanceSnapshot', JSON.stringify({ total: balance.total, free: balance.free, used: balance.used }));
      ...
      const assetKeys = new Set<string>([...Object.keys(free), ...Object.keys(used), ...Object.keys(total)]);

      const assets: AsterAccountAsset[] = Array.from(assetKeys).map((asset) => ({ asset, walletBalance: String(total[asset] ?? 0), availableBalance: String(free[asset] ?? 0), updateTime: now }));
      ...
      return {
         ...
         totalUnrealizedProfit: positions.reduce((acc, pos) => acc + Number(pos.unrealizedProfit ?? 0), 0).toString(),
         positions,
         assets,
      };
   }
   ...
   private mapBalanceToAccountSnapshotFromPositions(rawPositions: any[]): AsterAccountSnapshot {
      ...
      const positions = this.normalizePositions(rawPositions, now);
      this.logger('positions', JSON.stringify({ raw: rawPositions, mapped: positions }));
      const snapshot = this.lastBalanceSnapshot ?? { canTrade: true, canDeposit: true, canWithdraw: true, updateTime: now, totalWalletBalance: '0', totalUnrealizedProfit: '0', positions: [], assets: [] };
      return { ...snapshot, updateTime: now, totalUnrealizedProfit: positions.reduce((acc, pos) => acc + Number(pos.unrealizedProfit ?? 0), 0).toString(), positions };
   }

   private normalizePositions(rawPositions: any[], now: number): AsterAccountSnapshot['positions'] {
      return rawPositions.filter((pos) => pos).map((pos: any) => {
         const rawSymbol = pos?.symbol ?? pos?.instrument ?? pos?.market ?? pos?.info?.market ?? this.marketSymbol;
         const normalizedSymbol = rawSymbol === this.marketSymbol || rawSymbol === this.displaySymbol ? this.displaySymbol : String(rawSymbol ?? this.displaySymbol);
         ...
         const entryPrice = pos?.entryPrice ?? pos?.averageEntryPrice ?? pos?.info?.average_entry_price ?? pos?.entry_price ?? '0';
         const unrealized = pos?.unrealizedPnl ?? pos?.info?.unrealized_pnl ?? pos?.unrealized_profit ?? '0';

         return { symbol: normalizedSymbol, positionAmt: positionAmt.toString(), entryPrice: String(entryPrice ?? '0'), unrealizedProfit: String(unrealized ?? '0'), positionSide: 'BOTH' as const, updateTime: now };
      });
   }
   ...
   private mapOHLCVToKline(candle: CcxtOhlcv, interval: string): AsterKline {
      ...
      return { openTime: timestamp, closeTime: timestamp + this.getIntervalMs(interval), open: open.toString(), high: high.toString(), low: low.toString(), close: close.toString(), volume: volume.toString(), numberOfTrades: 0 };
   }

   private mapOrderTypeToCcxt(type: string): string {
      const typeMap: Record<string, string> = { LIMIT: 'limit', MARKET: 'market', STOP_MARKET: 'stop', TRAILING_STOP_MARKET: 'trailing-stop' };
      return typeMap[type] || 'limit';
   }

   private mapCcxtOrderTypeToAster(type: string | undefined): OrderType {
      const typeMap: Record<string, OrderType> = { limit: 'LIMIT', market: 'MARKET', stop: 'STOP_MARKET', 'trailing-stop': 'TRAILING_STOP_MARKET' };
      return type ? typeMap[type] ?? 'LIMIT' : 'LIMIT';
   }

   private normalizeInterval(interval: string): string {
      const map: Record<string, string> = { '1m': '1m', '3m': '3m', '5m': '5m', '15m': '15m', '30m': '30m', '1h': '1h', '4h': '4h', '1d': '1d' };
      return map[interval] ?? '1m';
   }

   private getIntervalMs(interval: string): number {
      const base: Record<string, number> = { '1m': 60 * 1000, '3m': 3 * 60 * 1000, '5m': 5 * 60 * 1000, '15m': 15 * 60 * 1000, '30m': 30 * 60 * 1000, '1h': 60 * 60 * 1000, '4h': 4 * 60 * 60 * 1000, '1d': 24 * 60 * 60 * 1000 };
   ...
   private isOrderClosed(order: AsterOrder): boolean {
      ...
      if (status.includes('CLOSE') || status.includes('FILLED') || status.includes('CANCEL') || status.includes('REJECT')) {
         return true;
      }
