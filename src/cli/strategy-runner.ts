...
const STRATEGY_FACTORIES: Record<StrategyId, StrategyRunner> = {
   trend: async (opts) => {
      ...
      await runEngine({ engine, strategy: 'trend', silent: opts.silent, getSnapshot: () => engine.getSnapshot(), onUpdate: (emitter) => engine.on('update', emitter), offUpdate: (emitter) => engine.off('update', emitter) });
   },
   maker: async (opts) => {
      const config = makerConfig;
      const adapter = createAdapterOrThrow(config.symbol);
      const engine = new MakerEngine(config, adapter);
      await runEngine({ engine, strategy: 'maker', silent: opts.silent, getSnapshot: () => engine.getSnapshot(), onUpdate: (emitter) => engine.on('update', emitter), offUpdate: (emitter) => engine.off('update', emitter) });
   },
