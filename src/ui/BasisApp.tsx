      ...
      <Box flexDirection='column' paddingX={1}>
         ...
         <Box flexDirection='row' marginBottom={1}>
            <Box flexDirection='column' marginRight={4}>
               ...
               {spotBalances.length ? (spotBalances.map((b) => <Text key={`spot-${b.asset}`}>{b.asset}: 可用 {formatNumber(b.free, 8)} ｜ 冻结 {formatNumber(b.locked, 8)}</Text>)) : <Text color='gray'>无</Text>}
            </Box>
            <Box flexDirection='column'>
               <Text color='cyan'>合约账户余额（非0）</Text>
               {futuresBalances.length ? (futuresBalances.map((b) => <Text key={`fut-${b.asset}`}>{b.asset}: 钱包 {formatNumber(b.wallet, 8)} ｜ 可用 {formatNumber(b.available, 8)}</Text>)) : <Text color='gray'>无</Text>}
            </Box>
         </Box>
         ...
         <Box flexDirection='column'>
            ...
            {lastLogs.length
               ? (lastLogs.map((entry, index) => {
                  ...
                  return <Text key={`${entry.time}-${index}`} color={color}>[{entry.time}] [{entry.type}] {entry.detail}</Text>;
               }))
