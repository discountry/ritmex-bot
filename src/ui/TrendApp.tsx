...
export function TrendApp({ onExit }: TrendAppProps) {
   ...
   const orderRows = sortedOrders.slice(0, 8).map((order) => ({ id: order.orderId, side: order.side, type: order.type, price: order.price, qty: order.origQty, filled: order.executedQty, status: order.status }));
   ...
   return (
      <Box flexDirection='column' paddingX={1} paddingY={0}>
         <Box flexDirection='column' marginBottom={1}>
            ...
            <Text>交易所: {exchangeName} ｜ 交易对: {snapshot.symbol} ｜ 最近价格: {formatNumber(lastPrice, 2)} ｜ SMA30: {formatNumber(sma30, 2)} ｜ 趋势: {trend}</Text>
         ...
         <Box flexDirection='row' marginBottom={1}>
            ...
            <Box flexDirection='column'>
               ...
               <Text>累计交易次数: {snapshot.totalTrades} ｜ 累计收益: {formatNumber(snapshot.totalProfit, 4)} USDT</Text>
               <Text>累计成交量: {formatNumber(sessionVolume, 2)} USDT</Text>
               {snapshot.lastOpenSignal.side ? <Text color='gray'>最近开仓信号: {snapshot.lastOpenSignal.side} @ {formatNumber(snapshot.lastOpenSignal.price, 2)}</Text> : null}
            </Box>
         </Box>

         <Box flexDirection='column' marginBottom={1}>
            <Text color='yellow'>当前挂单</Text>
            {orderRows.length > 0 ? <DataTable columns={orderColumns} rows={orderRows} /> : <Text color='gray'>暂无挂单</Text>}
         </Box>
