...
export function OffsetMakerApp({ onExit }: OffsetMakerAppProps) {
   ...
   const openOrderRows = sortedOrders.slice(0, 8).map((order) => ({ id: order.orderId, side: order.side, price: order.price, qty: order.origQty, filled: order.executedQty, reduceOnly: order.reduceOnly ? 'yes' : 'no', status: order.status }));
   ...
   const desiredRows = snapshot.desiredOrders.map((order, index) => ({ index: index + 1, side: order.side, price: order.price, amount: order.amount, reduceOnly: order.reduceOnly ? 'yes' : 'no' }));
   const desiredColumns: TableColumn[] = [{ key: 'index', header: '#', align: 'right', minWidth: 2 }, { key: 'side', header: 'Side', minWidth: 4 }, { key: 'price', header: 'Price', align: 'right', minWidth: 10 }, {
      key: 'amount',
      header: 'Qty',
      align: 'right',
      minWidth: 8,
   ...
   const imbalanceLabel = snapshot.depthImbalance === 'balanced' ? '均衡' : snapshot.depthImbalance === 'buy_dominant' ? '买盘占优' : '卖盘占优';
   ...
   return (
      <Box flexDirection='column' paddingX={1}>
         <Box flexDirection='column' marginBottom={1}>
            ...
            <Text>买10档累计: {formatNumber(snapshot.buyDepthSum10, 4)} ｜ 卖10档累计: {formatNumber(snapshot.sellDepthSum10, 4)} ｜ 状态: {imbalanceLabel}</Text>
            <Text color='gray'>当前挂单策略: BUY {snapshot.skipBuySide ? '暂停' : '启用'} ｜ SELL {snapshot.skipSellSide ? '暂停' : '启用'} ｜ 按 Esc 返回策略选择</Text>
         ...
         <Box flexDirection='row' marginBottom={1}>
            <Box flexDirection='column' marginRight={4}>
               ...
               {hasPosition
                  ? (
                     <>
                        <Text>方向: {snapshot.position.positionAmt > 0 ? '多' : '空'} ｜ 数量: {formatNumber(Math.abs(snapshot.position.positionAmt), 4)} ｜ 开仓价: {formatNumber(snapshot.position.entryPrice, 2)}</Text>
            ...
            <Box flexDirection='column'>
               ...
               <Text>累计成交量: {formatNumber(snapshot.sessionVolume, 2)} USDT</Text>
            </Box>
         </Box>
