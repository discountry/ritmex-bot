...
export function shouldStopLoss(position: PositionSnapshot, bestBid: number, bestAsk: number, lossLimit: number): boolean {
   ...
   const pnl = position.positionAmt > 0 ? (closePrice - position.entryPrice) * absPosition : (position.entryPrice - closePrice) * absPosition;
