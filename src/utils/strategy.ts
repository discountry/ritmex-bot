...
export function getPosition(snapshot: AsterAccountSnapshot | null, symbol: string): PositionSnapshot {
   ...
   const selected = withExposure.find((p) => p.positionSide === 'BOTH') ?? withExposure.sort((a, b) => Math.abs(Number(b.positionAmt)) - Math.abs(Number(a.positionAmt)))[0] ?? positions[0];
   const rawMark = Number(selected?.markPrice);
   const markPrice = Number.isFinite(rawMark) && rawMark > 0 ? rawMark : null;
   return { positionAmt: Number(selected?.positionAmt) || 0, entryPrice: Number(selected?.entryPrice) || 0, unrealizedProfit: Number(selected?.unrealizedProfit) || 0, markPrice };
}
...
export function computeBollingerBandwidth(values: AsterKline[], length: number, stdMultiplier: number): number | null {
 ...
 */
export function isOrderPriceAllowedByMark(params: { side: 'BUY' | 'SELL'; orderPrice: number | null | undefined; markPrice: number | null | undefined; maxPct: number }): boolean {
